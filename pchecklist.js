...
//list of bad passwords   
    ...
    stringOutput = "";
//this verifies the password is atleast 8 characters
//otherwise user will be sent a message saying it is less than 8 characters
if (stringPassword.length<8)
{
    stringOutput= "Your password length must be at least 8 characters.";
}
else
{
//this checks the password against the list of bad passwords
//if the password is in the list, they will be sent a message

   for (var i=0; i<arrayBadPasswords.length; i++)
   {
    if (stringPassword === arrayBadPasswords[i])
    {
        stringOutput = "This password is not strong enough. Please try again.";
        break;
    }
   }
}
//if the password passes the tests they will be sent a message telling them
if (stringOutput != "")
{
 ...
 {
   $("output").value= "This password is correct";
 }
};
//return to dom
